...
const WoofyHoverImage: React.FC<WoofyHoverImageProps> = ({
  ...
  const fragmentShader = `
    ...
    void main() {
      ...
      vec3 originalColor = tex1.rgb;
      vec3 effectColor;

      if (u_hasTexture2) {
        effectColor = texture2D(u_texture2, texCoord).rgb;
      } else {
      ...
      vec3 innerColor = effectColor;
      vec3 outerColor = originalColor;

      if (u_invertMask) {
        vec3 temp = innerColor;
        innerColor = outerColor;
        outerColor = temp;
      }

      vec3 finalColor = mix(outerColor, innerColor, mask);
  ...
  const initializeEffect = useCallback(() => {
    ...
    const runSetup = async () => {
      try {
        const texture1 = await loadTexturePromise(src);
        let texture2: THREE.Texture | null = null;
        if (hoverSrc) {
          texture2 = await loadTexturePromise(hoverSrc);
        }

        const img = texture1.image as any;
        ...
        const uniforms = {
          u_texture: { value: texture1 },
          u_texture2: { value: texture2 || texture1 },
          u_hasTexture2: { value: !!texture2 },
        ...
        console.error("Error setting up WoofyHoverImage:", err);
      }
    };
